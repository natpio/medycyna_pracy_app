...
def init_pdf(font_reg, font_bold, font_italic):
    ...
    # Ładowanie czcionek przekazanych z głównego pliku
...
def create_sanepid_pdf(orz_data, wizyta, pacjent, firma, signature_path, fonts):
    ...
    pdf.set_x(30)
    pdf.cell(100, 4, "(imię i nazwisko)")
    pdf.ln(4)
    ...
    pdf.multi_cell(0, 5, "1) wobec braku przeciwwskazań zdrowotnych zdolny(a) do podjęcia / wykonania* prac / rozpoczęcia nauki przy których istnieje możliwość przeniesienia zakażenia na inne osoby.")
    ...
    pdf.set_font("Roboto", size=10)
    pdf.set_xy(16, pdf.get_y())
    pdf.multi_cell(0, 5, "2) wobec przeciwwskazań zdrowotnych - niezdolny(a) do podjęcia / wykonywania* prac/ rozpoczęcia nauki przy których istnieje możliwość przeniesienia zakażenia na inne osoby, w procesie:")
    pdf.set_x(16)
    ...
    # Kod QR (Opcjonalnie, dla bezpieczeństwa tak jak w KBP)
