...
# Przygotowanie filtrów w pasku bocznym
...
# Wyodrębnienie danych wybranej firmy
firma_info = df_firmy[df_firmy['NazwaFirmy'] == wybrana_firma_label].iloc[0]
wybrany_nip = firma_info['NIP']
...
if raport_raw.empty:
    ...
    # 4. Mapowanie cen z cennika firmy
    cennik_map = {
        "Wstępne": firma_info['Cena_Wstepne'],
        "Okresowe": firma_info['Cena_Okresowe'],
        "Kontrolne": firma_info['Cena_Kontrolne'],
        "Sanitarno-Epidemiologiczne": firma_info['Cena_Sanepid']
    }
    
    raport_raw['Cena'] = raport_raw['TypBadania'].map(cennik_map)
    
    # --- KLUCZOWA POPRAWKA: Konwersja na liczby ---
    raport_raw['Cena'] = pd.to_numeric(raport_raw['Cena'], errors='coerce').fillna(0)
    
    # Dołączanie nazwisk pacjentów
    pacjenci_map = df_pacjenci.set_index('PESEL').to_dict('index')
    
    def get_pacjent_name(pesel):
        p = pacjenci_map.get(str(pesel))
        return f"{p['Nazwisko']} {p['Imie']}" if p else f"Nieznany ({pesel})"
    ...
    st.write(f"Okres rozliczeniowy: **{wybrany_miesiac}**")

    # Tabela końcowa
    tabela_finansowa = raport_raw[['DataWizyty', 'Pacjent', 'TypBadania', 'Cena']].copy()
    ...
    # Formatowanie z obsługą spacji jako separatora tysięcy
    total_formatted = f"{total:,.2f}".replace(",", " ")
    st.metric(label="Suma do zafakturowania (Netto)", value=f"{total_formatted} PLN")
