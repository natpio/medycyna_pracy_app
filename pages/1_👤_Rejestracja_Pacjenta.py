...
from db_service import add_patient_to_db, apply_pro_style, dekoduj_pesel
import datetime
...
if st.button("🪄 Uzupełnij dane z PESEL", use_container_width=True):
    d, p = dekoduj_pesel(pesel)
    if d: 
...
# Podział na dwie kolumny dla daty i płci
col_date, col_plec = st.columns(2)
with col_date:
...
with col_plec:
    # Wybór płci z uwzględnieniem wyliczeń z PESEL
    plec = st.radio("Płeć pacjenta:", options=["Mężczyzna", "Kobieta"], 
...
if st.button("💾 Zapisz Pacjenta w Bazie", type="primary", use_container_width=True):
    ...
    if not pesel or len(pesel) != 11 or not pesel.isdigit():
        ...
        # Zapis do bazy danych (8 argumentów, na końcu Płeć)
        sukces, msg = add_patient_to_db(pesel, imie, nazwisko, str(data_urodzenia), telefon, adres, email, plec)
        if sukces:
            st.success(f"✅ {msg}")
            st.balloons()
        else:
