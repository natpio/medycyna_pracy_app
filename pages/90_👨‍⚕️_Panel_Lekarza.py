...
# --- MODUŁ LIVE: Pływająca poczekalnia (Odświeża się automatycznie co 10 sek) ---
@st.fragment(run_every="10s")
def monitor_poczekalni():
    """Sprawdza w tle, czy recepcja dodała nowego pacjenta."""
    df_live = get_data_as_df("Wizyty")
    if not df_live.empty:
        oczekujacy = df_live[df_live['Status'] == 'Zaplanowana']
        ile = len(oczekujacy)
        if ile > 0:
            st.warning(f"🔔 **Status na żywo:** W poczekalni czeka **{ile}** pacjentów. (Kliknij odśwież poniżej, aby zaktualizować listę wyboru)")
        else:
            st.success("🎉 **Status na żywo:** Brak oczekujących pacjentów w poczekalni.")

# Wywołanie monitora na górze strony
monitor_poczekalni()

# Przycisk do ręcznego przeładowania listy rozwijanej (żeby lekarz widział nowych pacjentów)
if st.button("🔄 Odśwież listę pacjentów", use_container_width=True):
    st.rerun()
...
if wybrana_etykieta != "--- Wybierz pacjenta ---":
    ...
    st.markdown("### 📝 Decyzja Orzecznika")
